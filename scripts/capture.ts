...
const toolsToScan = tools
	...
	.filter(
		(tool: TTool) => !onlyMissing || !existsSync(`src/images/${tool.slug}.png`),
	);
...
async function bootstrap() {
	...
	console.log(
		"When done, come back here and press Enter (do NOT Ctrl-C — profile won't flush).",
	);
...
async function capture() {
	...
	console.log(
		`${toolsToScan.length} tool(s) to capture${onlyMissing ? " (missing only)" : ""}.`,
	);
	...
	for (const t of toolsToScan) {
		...
		await page
			...
			.catch((e) =>
				console.warn(`  (goto: ${e.message.split("\n")[0]} — continuing)`),
			);
		...
		const input = await waitForEnter(
			`  Navigate/log in as needed, then press Enter to capture ${t.slug} (s+Enter to skip)…`,
		);
